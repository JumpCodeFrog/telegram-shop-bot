...
func (b *Bot) handleAdmin(msg *tgbotapi.Message) {
	...
	text := b.t(lang, "admin_panel_title")
...
func (b *Bot) handleAddProduct(msg *tgbotapi.Message) {
	...
	b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_add_product_name")))
}
...
func (b *Bot) handleAddProductStep(msg *tgbotapi.Message) bool {
	...
	if msg.Text == "/cancel" {
		...
		if state != nil {
			b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_cancel")))
			return true
		}
		return false
	}
	...
	case storage.StepName:
		...
		b.send(tgbotapi.NewMessage(chatID, b.t(lang, "admin_add_product_desc")))
	...
	case storage.StepPriceUSD:
		...
		if err != nil || p <= 0 {
			b.send(tgbotapi.NewMessage(chatID, b.t(lang, "admin_error_invalid_price")))
			return true
		}
		state.PriceUSD = p
	...
	case storage.StepStock:
		...
		if err != nil || s < 0 {
			b.send(tgbotapi.NewMessage(chatID, b.t(lang, "admin_error_invalid_stock")))
			return true
		}
		state.Stock = s
		state.Step = storage.StepPhoto
		_ = b.fsm.SetAddProductState(ctx, msg.From.ID, state, 30*time.Minute)
		b.send(tgbotapi.NewMessage(chatID, b.t(lang, "admin_add_product_photo")))
	...
	case storage.StepCategory:
		...
		if err != nil {
			b.send(tgbotapi.NewMessage(chatID, b.t(lang, "admin_error_invalid_category_id")))
			return true
		}
...
func (b *Bot) sendAdminProductDetails(chatID int64, product *storage.Product, lang string) {
	toggleLabel := b.t(lang, "admin_btn_toggle_off")
	if product.Stock <= 0 {
		toggleLabel = b.t(lang, "admin_btn_toggle_on")
	}

	text := fmt.Sprintf(b.t(lang, "admin_product_details"),
...
func (b *Bot) handleEditProduct(msg *tgbotapi.Message) {
	...
	if err != nil {
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_edit_product_usage")))
		return
	}
...
func (b *Bot) handleEditProductField(msg *tgbotapi.Message, prodID int64, field, value string) {
	...
	if strings.TrimSpace(value) == "" {
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_error_value_required")))
		return
	}
	...
	case "active":
		...
		if err != nil {
			b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_error_invalid_bool")))
			return
		}
		product.IsActive = active
	default:
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_error_unsupported_field")))
		return
	}

	if err := b.products.UpdateProduct(ctx, product); err != nil {
		b.logger.Error("update product", "product_id", prodID, "error", err)
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_error_update_product")))
		return
	}
...
func (b *Bot) handleDeleteProduct(msg *tgbotapi.Message) {
	...
	id, err := strconv.ParseInt(strings.TrimSpace(msg.CommandArguments()), 10, 64)
	if err != nil {
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_delete_product_usage")))
		return
	}
	if err := b.products.DeleteProduct(context.Background(), id); err != nil {
		b.logger.Error("delete product", "product_id", id, "error", err)
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_error_delete_product")))
		return
	}
...
func (b *Bot) handleAddCategory(msg *tgbotapi.Message) {
	...
	if len(args) < 2 {
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_add_category_usage")))
		return
	}
	...
	if err != nil {
		...
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_error_create_category")))
		return
	}
...
func (b *Bot) handleEditCategory(msg *tgbotapi.Message) {
	...
	args := strings.Fields(msg.CommandArguments())
	if len(args) < 3 {
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_edit_category_usage")))
		return
	}
	...
	default:
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_error_category_fields")))
		return
	}

	if err := b.catalog.UpdateCategory(ctx, category); err != nil {
		b.logger.Error("update category", "category_id", categoryID, "error", err)
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_error_update_category")))
		return
	}
...
func (b *Bot) handleDeleteCategory(msg *tgbotapi.Message) {
	...
	id, err := strconv.ParseInt(strings.TrimSpace(msg.CommandArguments()), 10, 64)
	if err != nil {
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_delete_category_usage")))
		return
	}
	if err := b.catalog.DeleteCategory(context.Background(), id); err != nil {
		b.logger.Error("delete category", "category_id", id, "error", err)
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_error_delete_category")))
		return
	}
...
func (b *Bot) handleListCategories(msg *tgbotapi.Message) {
	...
	if err != nil {
		...
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_error_load_categories")))
		return
	}
	if len(categories) == 0 {
...
func (b *Bot) handleOrdersAll(msg *tgbotapi.Message) {
	...
	if err != nil {
		...
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_error_load_orders")))
		return
	}
	if len(orders) == 0 {
...
func (b *Bot) handleSetDelivered(msg *tgbotapi.Message) {
	...
	id, err := strconv.ParseInt(strings.TrimSpace(msg.CommandArguments()), 10, 64)
	if err != nil {
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_set_delivered_usage")))
		return
	}
	order, err := b.order.SetDelivered(context.Background(), id)
	if err != nil {
		b.logger.Error("set delivered", "order_id", id, "error", err)
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_error_set_delivered")))
		return
	}
	b.send(tgbotapi.NewMessage(msg.Chat.ID, fmt.Sprintf(b.t(lang, "admin_order_delivered"), order.ID)))
...
func (b *Bot) handleAnalytics(msg *tgbotapi.Message) {
	b.sendAnalytics(msg.Chat.ID, msg.From.LanguageCode, 0, 7)
}
...
func (b *Bot) handleAnalyticsCallback(chatID int64, msgID int, data, lang string) {
	...
	b.sendAnalytics(chatID, lang, msgID, days)
}

func (b *Bot) sendAnalytics(chatID int64, lang string, msgID int, days int) {
	ctx := context.Background()

	summary, err := b.analytics.GetRevenueSummary(ctx)
	if err != nil {
		b.logger.Error("analytics summary", "error", err)
		b.send(tgbotapi.NewMessage(chatID, b.t(lang, "admin_error_load_analytics")))
		return
	}
	...
	sb.WriteString(fmt.Sprintf(b.t(lang, "admin_analytics_revenue"), periodUSD, periodStars, periodOrders))
	...
	if len(topProducts) == 0 {
		sb.WriteString(b.t(lang, "admin_analytics_no_orders"))
	} else {
	...
	sb.WriteString(b.t(lang, "admin_analytics_payment_methods"))
...
func (b *Bot) handleListPromos(msg *tgbotapi.Message) {
	...
	var sb strings.Builder
	sb.WriteString(b.t(lang, "admin_promos_title"))
...
func (b *Bot) handleExportOrders(msg *tgbotapi.Message) {
	...
	if err := writer.Error(); err != nil {
		...
		b.send(tgbotapi.NewMessage(msg.Chat.ID, b.t(lang, "admin_error_csv_export")))
		return
	}
...
func (b *Bot) onAdminToggleStock(chatID int64, data, lang string) {
	...
	if err != nil {
		...
		b.send(tgbotapi.NewMessage(chatID, b.t(lang, "admin_error_identify_product")))
		return
	}
	...
	if err := b.products.UpdateProduct(ctx, product); err != nil {
		...
		b.send(tgbotapi.NewMessage(chatID, b.t(lang, "admin_error_update_product")))
		return
	}
...
func (b *Bot) sendBtnStyleList(chatID int64, msgID int, lang string) {
	...
	sb.WriteString(b.t(lang, "admin_btnstyle_hint"))
	for _, key := range AllButtonKeys {
		style := ButtonStyle(stored[key])
		sb.WriteString(fmt.Sprintf("%s %s → %s\n", StyleEmoji(style), ButtonKeyLabel(key, lang, b), styleLabel(style)))
	}
	...
	for i := 0; i < len(AllButtonKeys); i += 2 {
		...
		for j := i; j < i+2 && j < len(AllButtonKeys); j++ {
			key := AllButtonKeys[j]
			style := ButtonStyle(stored[key])
			label := fmt.Sprintf("%s %s", StyleEmoji(style), ButtonKeyLabel(key, lang, b))
...
func (b *Bot) sendBtnStylePicker(chatID int64, msgID int, key, lang string) {
	ctx := context.Background()
	current, _ := b.uiSettings.GetButtonStyle(ctx, key)

	text := fmt.Sprintf(
		b.t(lang, "admin_btnstyle_picker"),
		ButtonKeyLabel(key, lang, b),
	...
	styleOptions := []struct {
		...
		{b.t(lang, "admin_btnstyle_primary"), StylePrimary},
		{b.t(lang, "admin_btnstyle_success"), StyleSuccess},
		{b.t(lang, "admin_btnstyle_danger"), StyleDanger},
...
func (b *Bot) onAdminSetStyle(chatID int64, msgID int, data, lang string) {
	...
	if err := b.uiSettings.SetButtonStyle(ctx, key, style); err != nil {
		...
		b.send(tgbotapi.NewMessage(chatID, b.t(lang, "admin_error_save_style")))
		return
	}
