...
// CreateOrder inserts an order and its items.
func (s *SQLOrderStore) CreateOrder(ctx context.Context, order *Order, items []OrderItem) (int64, error) {
	var orderID int64
	err := WithinTransaction(ctx, s.db, func(ctx context.Context) error {
		...
		if err != nil {
			return fmt.Errorf("order store: last insert id: %w", err)
		}
		orderID = id
		...
		for _, item := range items {
			...
			if err != nil {
				return fmt.Errorf("order store: insert order item: %w", err)
			}
		}
		return nil
	})
